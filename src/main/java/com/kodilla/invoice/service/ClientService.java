...
public class ClientService {
    ...
    public Client save(final Client client) {
        ofNullable(client).ifPresent(customer->emailService.send(new Mail(adminConfig.getAdminMail(), SUBJECT,
                "The customer: "+ customer.getName() + " has been created, and added to database only. Don't forget sending it to external services, in case using it by another users.")));
        assert client != null;
    ...
    public void deleteById(Long id) {
        ...
        emailService.send(new Mail(adminConfig.getAdminMail(), SUBJECT_DELETE,
                "The customer: "+ id + " has been deleted from database only. Don't forget delete it from external services."));
    }
